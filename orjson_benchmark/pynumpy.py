...
PYNUMPY_DIR = Path("doc/types/numpy")
ITERATIONS = 10
...
def test_numpy():
    ...
    headers = ("Library", "Version", "Latency (ms)", "RSS diff (MiB)", "vs. orjson")
    ...
    buf = io.StringIO()
    buf.write(f"**{kind}** - {output_in_mib:,.1f}MiB output (orjson)\n\n")
    buf.write(tabulate(table, headers, tablefmt="rst") + "\n")

    dump_string_io_to_file(PYNUMPY_DIR / f"benchmark_{kind}.rst", buf)
    print(buf.getvalue())
